...
public class RulesOf6005 {
	...
	public static boolean mayUseCodeInAssignment(boolean writtenByYourself,
	    ...
	    // 1. If the code is written by yourself, you can use it.
	    if (writtenByYourself) {
	        return true;
	    }
	    
	    // 2. If the code is not written by you, check other conditions.
	    if (!writtenByYourself) {
	        // 2a. If it's available to others and was written as coursework,
	        // you must cite your source to use it.
	        if (availableToOthers && writtenAsCourseWork) {
	            return citingYourSource;
	        }
	        
	        // 2b. If it's available to others but not coursework, you cannot use it.
	        if (availableToOthers && !writtenAsCourseWork) {
	            return false; 
	        }
	        
	        // 2c. If it's not available to others, check if it was coursework and properly cited.
	        if (!availableToOthers) {
	            return writtenAsCourseWork && citingYourSource;
	        }
	    }
	    
	    // 3. If the assignment specifically requires implementation and the code is not yours, you cannot use it.
